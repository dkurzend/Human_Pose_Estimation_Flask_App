...
class Residual(nn.Module):
    ...
    def forward(self, x):
        # in the paper figure 4 left we see that the input is added to the output within a residual module (residual connection)
...
class SoftGatedSkipConnection(nn.Module):
    def __init__(self, inp_dim, out_dim):
        ...
        # for each channel we have one parameter
        # as in paper alphas are initialized with zero
        self.alpha = nn.Parameter(torch.zeros((1,out_dim, 1, 1))) # check the correct shape
